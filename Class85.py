...
print(f"Seat to Find: {target_seat}\n")
...
def binary_search(seats, target):
    ...
    while low <= high:
        steps = steps + 1
        mid = (low + high) // 2
        print(f"Checking middle seat: {seats[mid]}")
...
if index != -1:
    print(f"Seat found at index: {index}")
else:
...
def recursive_binary_search(seats, target, low, high):
    ...
    mid = (low + high) // 2
    print(f"Recursive check: {seats[mid]}")
...
if recursive_index != -1:
    print(f"Seat found at index: {recursive_index}")
else:
